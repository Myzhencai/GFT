    ...
    def error(self):
        with tf.name_scope('loss'):
            if not self._loss:
                ...
                denominator = tf.reduce_sum(self.output*self.output + self._prediction*self._prediction+0.00000006, axis=(1,2,3))
                self.dice = tf.reshape(1 - (numerator / denominator), (-1, 1, 1, 1))
                print("dice", self.dice.shape)
                self.loss1 = tf.nn.weighted_cross_entropy_with_logits(targets=self.output, logits=self._prediction,
                                                                      pos_weight=-0.2)
                self.loss2 = tf.reduce_sum((self.output-self._prediction)*(self.output-self._prediction))
                self._loss = tf.reduce_mean(self.loss1 + self.dice+self.loss2)
                ##################################################################################################################
